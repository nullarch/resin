...
데이터를 쓸 수 있습니다. clone 없이도 같은 CLI를 쓸 수 있습니다: `npx @nullarch/resin build my.pine`.
...
그 결과 하나가 소스 안에 남아 있고, 읽기 시작하기 전에 알아두는 게 좋습니다:

- **컴파일러·런타임 에러 메시지 365개는 전부 영어입니다** — 한국어로 시작했지만, 이 목록의
  다른 무엇보다 먼저 번역됐습니다. 도구를 *읽는* 것이 아니라 *쓰는* 것에 영향을 주는 유일한
  항목이었기 때문입니다. 3,000개 스크립트 차분 리플레이를 포함한 전체 테스트 스위트가 이
  변경의 동작 변화가 0임을 고정했습니다.
